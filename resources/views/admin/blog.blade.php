                        ...
                        <div class="col-12">
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <h3 class="card-title">Input Blog</h3>
                                </div>
                                <!-- Notif -->
                                ...
                                <!-- Notif -->
                                <div class="card-header">
                                    <a href="{{ route('blog-view.create') }}" class="btn btn-primary">Input Blog Baru</a>
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body">
                                    <table id="example2" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$i+1}}</td>
                                                <td>{{$item->judul}}</td>
                                                <td>{{$item->categoriModels->categori}}</td>
                                                <td>{{$item->tgl}}</td>
                                                <td>{!! $item->blog !!}</td>
                                                <td><img src="{{ URL::to('/') }}/fotoblog/{{ $item->foto_blog}}" class="img-thumbnail" width="75" /></td>
                                                <td><a href="{{ route('blog-view.edit',$item->id) }}" class="btn btn-sm btn-success">Edit</a>
                                                    <form action="{{ route('blog-view.destroy', $item->id) }}" method="post">
                                                        {{ csrf_field() }}
                                                        {{ method_field('DELETE') }}
                                                        <button class="btn btn-sm btn-danger" type="submit" onclick="return confirm('Yakin ingin menghapus data {{ $item->judul }} ?')">Hapus</button>
                                                    </form>
                                                </td>
                                            </tr>
